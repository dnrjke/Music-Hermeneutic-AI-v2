...
#### 3-e. gate 소니파이 청취 — norm 잔여가 유효

`전체_gate+norm잔여_클릭.wav` 청취 결과 (사용자):
- **잔여가 많다** — 75% 이상이 높은 신호음(5kHz, norm에만 있는 피크)
- VN 3:08-3:24: 앞부분 낮은 신호음(gate base) → 도중에 높은 신호음으로 전환 → 뒤는 복합적
- 의미: **gate만으로는 VN 경계 블록에서 과소탐지. norm 잔여가 실제 사건.**

수치 확인:
- SS: gate+norm잔여=584 (잔여=26) ≈ Otsu(558) — 거의 변화 없음
- VN: gate+norm잔여=917 (잔여=378) ≈ norm(908) — norm과 거의 동일

→ `gate+norm잔여`는 사실상 **SS→Otsu, VN→norm의 자동 적응 결과**.

#### 3-f. Fable 제안 — 전체 파이프라인 구조
