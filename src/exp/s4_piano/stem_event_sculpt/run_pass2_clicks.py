"""Mono click sonify for pass2 click candidates (Dir-style overlay)."""
...
# Fixed D-21: peak pick on sculpt residue envelope (not SuperFlux / not 395).
CLICK_PARAMS = {
    "rms_win": N_FFT,
    "rms_hop": HOP,
...
CANDIDATES = (
    ("perc_attack_release_removed.wav", "attack_release_removed"),
    ("perc_soft_gate.wav", "soft_gate"),
    ("perc_attack_release.wav", "attack_release"),
)
...
def run_once() -> dict[str, Any]:
    ...
    for wav_name, key in CANDIDATES:
        src = PASS2_DIR / wav_name
        if not src.exists():
        ...
        out_path = PASS2_DIR / out_name
        entry = write_listening_wav(out_path, overlaid, SR)
        files[out_name] = {
            **entry,
            "role": "pass2_click_sonify",
            "source_wav": wav_name,
            "candidate": key,
    ...
    manifest = {
        "experiment": "stem_event_sculpt_pass2_clicks",
        "fixed_rules": {
    ...
    write_json(PASS2_DIR / "pass2_clicks_manifest.json", manifest)
    return manifest
...
def determinism_check(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    mismatches = [
        n
        for n in first
        if first[n] != second["files"].get(n, {}).get("sha256")
    ]
    peak_mismatch = [
        k
    ...
    write_json(PASS2_DIR / "pass2_clicks_determinism.json", report)
    return report
