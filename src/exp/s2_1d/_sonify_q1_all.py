"""전 트랙 Q1 rescue 청각 검증 파일 생성.

구조:
  out/sonify/{트랙약칭}/{구간}/
    원곡.wav
    1차탐지_클릭.wav        — Otsu 초과 피크 (3kHz)
...
for p in audio_paths():
    ...
    pk_all, rescued_times = q1_rescue(env, bands, times, dur)
    ...
    for s0_s, s1_s in segs:
        ...
        base_in = pk_base[(pk_base >= s0_s) & (pk_base < s1_s)]
        all_in = pk_all[(pk_all >= s0_s) & (pk_all < s1_s)]

        # 원곡
        sf.write(str(seg_dir / "원곡.wav"), seg, SR)
        ...
        sf.write(str(seg_dir / "1차탐지_클릭.wav"),
                 np.clip(out, -1, 1), SR)
        ...
        for t in all_in:
            idx = int((t - s0_s) * SR)
            c = click_rescue if t in rescued_times else click_base
            end = min(idx + len(c), len(out))
            n = end - idx
            if n > 0:
                out[idx:end] += c[:n]
        sf.write(str(seg_dir / "Q1전체_클릭.wav"),
                 np.clip(out, -1, 1), SR)
        ...
        for t in all_in:
            if t in rescued_times:
                ...
                n_resc += 1
        sf.write(str(seg_dir / "Q1구조만_클릭.wav"),
                 np.clip(out, -1, 1), SR)

        print(f"  {alias}/{seg_label}/  1차={len(base_in)}  Q1={len(all_in)}  구조={n_resc}")
